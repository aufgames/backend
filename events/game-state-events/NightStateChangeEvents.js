 ...
 */
function startNight(io, socket, aufGame) {
    ...
    socket.on('night-vote', () => {
        const { roomID } = socket.player;
        const room = aufGame.gameRoomsDict[roomID];
        const numLiveImpostor = room.players
            .filter((player) => player.role === PlayerRole.IMPOSTOR)
            .filter((player) => player.status === PlayerStatus.ALIVE).length;
        const numImpostorVotes = Object.keys(room.voteHandler.impostorVoteMap).length;
        ...
        const numLiveDetective = room.players
            ...
            .filter((player) => player.status === PlayerStatus.ALIVE).length;
        if (numLiveImpostor === numImpostorVotes) {
 ...
 * @param {AUFGame} aufGame
 */
function endNight(io, socket, aufGame) {
    const { roomID } = socket.player;
    const room = aufGame.gameRoomsDict[roomID];

    const playerKilled = room.voteHandler.getImpostorVotedPlayer();
    let usernameKilled;

    if (playerKilled) {
        usernameKilled = room.getPlayerByNickname(playerKilled).pwallet;
        room.getPlayerByNickname(playerKilled).status = PlayerStatus.KILLED_BY_IMPOSTOR;
    }

    const winningRole = room.getWinningRole();

    if (winningRole !== null) {
        io.in(roomID).emit('night-end', new NightEndDTO(playerKilled, true, usernameKilled));
        io.in(roomID).emit(
            'game-over',
            new GameOverDTO(
                winningRole,
                room.getPlayersByRole(winningRole),
                room.getPlayersByRole(winningRole).length,
            )
        );
    } else {
        io.in(roomID).emit('night-end', new NightEndDTO(playerKilled, false, usernameKilled));
    }
