...
class VoteHandler {
    ...
    /**
     * Gets the player voted for by the impostor, based on the impostor vote map.
     * Compares the voted player with the player that the medic has chosen to save, to check if they are the same player.
     * @returns The player that has been killed by the impostor, or null if the medic has saved the right player.
     */
    getImpostorVotedPlayer() {
        const impostorChosenPlayer = this.getVotedPlayer(this.impostorVoteMap);
        if (this.medicChosenPlayer && impostorChosenPlayer === this.medicChosenPlayer.nickname) {
            return null;
        }
        return impostorChosenPlayer;
    }
