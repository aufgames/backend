...
class Room {
    ...
    /**
     * Gets the winning players for the game, given a role. If the specified role is CREWMATE, medics and detectives are included.
     * @param {Role} role The winning role: either CREWMATE, IMPOSTOR or JESTER
     * @returns The list of winning players , depending on the winning role provided.
     */
    getWinningPlayers(role) {
        let playersOfTheRole = [];
        if (role === roles.CREWMATE) {
            playersOfTheRole = [
                ...this.getPlayersByRole(roles.CREWMATE),
     ...
     */
    getWinningRole() {
        const numOfImpostorAlive = this.getPlayersByRole(roles.IMPOSTOR).filter(
            (player) => player.status === PlayerStatus.ALIVE
        ).length;

        const numOfCrewmatesAlive = [
            ...this.getPlayersByRole(roles.CREWMATE),
            ...this.getPlayersByRole(roles.MEDIC),
            ...this.getPlayersByRole(roles.DETECTIVE),
        ...
        if (numOfImpostorAlive === 0) {
            return roles.CREWMATE;
        }
        if (numOfImpostorAlive >= numOfCrewmatesAlive) {
