...
describe('night time vote event tests', () => {
     ...
     * It goes through stariting a game to checking impostor vote
     *
     */
    ...
    /**
     * This function votes for a player and checks if they are impostors
     * @returns A promise resolving on the suspect reveal
     */
    async function nightTimeVote(hostRole) {
        return new Promise((resolve) => {
            let isImpostor = false;
            // check if host is impostor
            if (hostRole === RoleEnum.IMPOSTOR) {
                isImpostor = true;
            }
            ...
            clientSockets[1].on('suspect-reveal', (suspectRevealDTO) => {
                ...
                expect(suspectRevealDTO.isImpostor).toBe(isImpostor);
                resolve();
            });
            // check if leon is impostor or jester
